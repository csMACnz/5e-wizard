...
- FR1.5: Background selection; grant proficiencies and feature references.
  - FR1.5.1 (Language Selection UI): The Background step shall display a **Languages** section after background is selected. The section shall show fixed race-granted languages (read-only) and an interactive pick list for extra language slots. The section is hidden until a background is chosen.
  - FR1.5.2 (Fixed languages): Fixed languages are derived from the selected race's `languageIds` (and the selected subrace's `languageIds` if applicable). These are displayed as read-only chips and do not consume extra-language slots.
  - FR1.5.3 (Extra language slots): The number of extra language slots is computed as `background.languageCount + count(trait:extra-language in race and subrace traitIds)`. Each `trait:extra-language` in the race's or subrace's `traitIds` contributes +1 extra slot.
  - FR1.5.4 (Pick list constraints): The pick list presents all languages from `data/languages.json` excluding those already in the fixed list. A user cannot select a language already fixed, cannot select duplicates, and cannot exceed the computed slot count.
  - FR1.5.5 (Validation): When `extraLanguageSlots > 0`, the Background step shall be considered invalid until exactly `extraLanguageSlots` extra languages have been chosen. Validation errors use the code `ERR_LANGUAGE_PICKS_INCOMPLETE` (too few picks) or `ERR_LANGUAGE_PICKS_EXCESS` (too many picks).
  - FR1.5.6 (Storage): `Character.Proficiencies.Languages` shall store the **resolved list** of language IDs: `fixedLanguages ∪ chosenExtraLanguages` (deduped), using `lang:*` IDs (e.g., `lang:common`).
  - FR1.5.7 (Display): The print sheet shall resolve `lang:*` IDs to human-readable display names using `data/languages.json` for the Languages section under Proficiencies.
  - FR1.5.8 (Canonical language data): `data/languages.json` shall provide all SRD 5.1 standard languages (Common, Dwarvish, Elvish, Giant, Gnomish, Goblin, Halfling, Orc) and exotic languages (Abyssal, Celestial, Draconic, Deep Speech, Infernal, Primordial, Sylvan, Undercommon), each with an `id` (`lang:*`) and `displayName`.
  - FR1.5.9 (Reconciliation): When the selected race or background changes after language picks have been made, the system shall recompute fixed languages and slot count, infer chosen extras as `storedLanguages - fixedLanguages`, trim deterministically if over the new limit, and rebuild the stored list. No corruption of previously valid picks shall occur.
...
- FR7.2: Automatic persistence to browser local storage.
  ...
  - FR7.2.5: The `schemaVersion` field shall start at `1` and shall be incremented whenever the persisted session format changes in a backward-incompatible way. When loading a session, the application shall reject (return null / skip) any session whose `schemaVersion` does not match the current supported version, ensuring stale or future-format data is never silently mis-read. Current supported version: `3` (bumped from `2` when `Character.HitPointEntries` was added).
